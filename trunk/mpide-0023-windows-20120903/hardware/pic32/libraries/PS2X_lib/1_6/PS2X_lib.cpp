...
#if defined(__AVR__)
#define SaveInterruptState {uint8_t old_sreg = sreg;}
#define DisableInterrupts   cli();
#define RestoreInterrupts	{SREG = old_sreg;}
#else
#define SaveInterruptState 
...
inline void  PS2X::CMD_CLR(void) {
   CLR(*_cmd_oreg,_cmd_mask); // SET(*_cmd_oreg,_cmd_mask);
}

inline void  PS2X::ATT_SET(void) {
  SET(*_att_oreg,_att_mask); // low enable joystick
}
...
inline bool PS2X::DAT_CHK(void) {
	return CHK(*_dat_ireg,_dat_mask);
}

#else
inline void  PS2X::CLK_SET(void) {
	*_clk_lport |= _clk_mask;
}
